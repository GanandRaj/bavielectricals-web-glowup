                ...
                {services.map((service, index) => (
                  ...
                  >
                    <div className={`${service.bgColor} ${service.hoverBg} w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-all duration-300 group-hover:scale-110`}>
                      <service.icon className={`h-6 w-6 bg-gradient-to-br ${service.color} bg-clip-text text-transparent`} style={{
                        background: `linear-gradient(135deg, ${service.color.split(' ')[0].replace('from-', '')}, ${service.color.split(' ')[2].replace('to-', '')})`,
